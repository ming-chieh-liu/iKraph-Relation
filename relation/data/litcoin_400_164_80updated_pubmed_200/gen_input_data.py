...
def main():
    ...
    pubmed_data = pubmed_data.drop(columns=['batch']).reset_index(drop=True)

    combined_data = pd.concat([litcoin_data, pubmed_data], ignore_index=True)

    output_file = "./new_annotated_train_litcoin_400_164_80updated_pubmed_200_pd.json"
    ...
    for split_id, this_pmids in enumerate(pmids_per_split):
        os.makedirs(os.path.join(f"new_train_splits_litcoin_400_164_80updated_pubmed_200/split_{split_id}"), exist_ok=True)
        ...
        output_data_fp = os.path.join(f"new_train_splits_litcoin_400_164_80updated_pubmed_200/split_{split_id}/data.json")
        this_data.reset_index(drop=True).to_json(open(output_data_fp, "w"), indent=4, orient="table")
        output_copylist_fp = os.path.join(f"new_train_splits_litcoin_400_164_80updated_pubmed_200/split_{split_id}/copy_list.csv")
