...
def main():
    litcoin_pubmed_train = pd.read_json("../litcoin_400_164_80updated_pubmed_100_100/new_annotated_train_litcoin_400_164_80updated_pubmed_100_pd.json", orient='table')
    
    pubmed_data = pd.read_json("../litcoin_400_164_80updated_pubmed_100_100/new_annotated_test_pubmed_100_pd.json", orient='table')
    ...
    sampled_batch_2 = random.sample(list(batch_2_pmids), int(n_batch_2 * sample_ratio))
    sampled_batch_3 = random.sample(list(batch_3_pmids), int(n_batch_3 * sample_ratio))
    ...
    # Save test data
    pubmed_test_output = "./new_annotated_test_pubmed_40_pd.json"
    ...
    combined_data = pd.concat([litcoin_pubmed_train, pubmed_train.drop(columns=['batch'])], ignore_index=True)

    output_file = "./new_annotated_train_litcoin_400_164_80updated_pubmed_160_pd.json"
    ...
    for split_id, this_pmids in enumerate(pmids_per_split):
        os.makedirs(os.path.join(f"new_train_splits_litcoin_400_164_80updated_pubmed_160/split_{split_id}"), exist_ok=True)
        ...
        output_data_fp = os.path.join(f"new_train_splits_litcoin_400_164_80updated_pubmed_160/split_{split_id}/data.json")
        this_data.reset_index(drop=True).to_json(open(output_data_fp, "w"), indent=4, orient="table")
        output_copylist_fp = os.path.join(f"new_train_splits_litcoin_400_164_80updated_pubmed_160/split_{split_id}/copy_list.csv")
