...
# coding: utf-8
"""
Pure Accelerate training script for multi-GPU distributed training.
...
def main():
    ...
    #     model_dtype = torch.float16 # let the trainer to handle fp16 loading
    else:
        model_dtype = torch.float32  # Explicitly use fp32 for FSDP compatibility

    # Model loading priority:
    # 1. MXFP4 models - Cannot use LoRA, must dequantize
    # 2. QLoRA if enabled - 4-bit quantization + LoRA adapters
    # 3. Regular loading - Full precision or mixed precision
    if is_mxfp4_model(PRE_TRAINED_MODEL_NAME):
        # MXFP4 models (e.g., gpt-oss-20b) CANNOT use LoRA
        # Must dequantize to trainable format
        if non_train_args.use_qlora:
            logger.warning(f"MXFP4 model detected. QLoRA is not supported for MXFP4 models. Using MXFP4 dequantization instead.")
        ...
        # QLoRA: 4-bit quantization + LoRA (for non-MXFP4 models)
