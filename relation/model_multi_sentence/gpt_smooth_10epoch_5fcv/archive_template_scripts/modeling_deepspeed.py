...
"""
DeepSpeed-enabled version of modeling.py for distributed training.
Supports ZeRO Stage 2 and Stage 3 for training large models like GPT-OSS-20B.

Usage:
    deepspeed --num_gpus=4 modeling_parallel.py config.json

Config should include:
    "deepspeed": "ds_config_zero2.json"  # or ds_config_zero3.json
"""
...
class SaveBestModelCallback(TrainerCallback):
    ...
    Note: With DeepSpeed ZeRO Stage 3 and `stage3_gather_16bit_weights_on_model_save: true`
    in config, save_pretrained() automatically gathers weights from all ranks.
    """
    ...
    def on_evaluate(self, args, state, control, metrics=None, **kwargs):
        ...
        # Save checkpoint - Trainer's processing_class handles DeepSpeed automatically
        # when using model.save_pretrained() with ZeRO Stage 3, weights are gathered
...
def main():
    ...
    # Find JSON config file in argv (DeepSpeed adds --local_rank=X before it)
    json_file = None
    ...
    if json_file:
        ...
        non_train_args, data_args, training_args = parser.parse_json_file(json_file=os.path.abspath(json_file))
    else:
    ...
    # For ZeRO Stage 3, must create HfDeepSpeedConfig before model loading
    # This tells HuggingFace to coordinate with DeepSpeed for weight initialization
    dschf = None
    if training_args.deepspeed:
        dschf = HfDeepSpeedConfig(training_args.deepspeed)

    # Load model - HuggingFace Trainer handles ZeRO-3 partitioning automatically
    ...
    # Only move to device if NOT using DeepSpeed (DeepSpeed handles device placement)
    ...
    if save_best_callback.best_checkpoint_path is not None:
        ...
        if model.config.pad_token_id is None:
            model.config.pad_token_id = tokenizer.pad_token_id
        # Only move to device if NOT using DeepSpeed
        if not training_args.deepspeed:
            model = model.to(device)
        trainer.model = model
