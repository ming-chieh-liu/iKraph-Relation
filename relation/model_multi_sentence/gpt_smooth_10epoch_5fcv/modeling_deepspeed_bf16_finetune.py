...
# coding: utf-8
"""
DeepSpeed full fine-tuning script for regular bf16/fp16 models (e.g., Llama-3.1-8B-Instruct).

Fully fine-tunes a standard HuggingFace model with DeepSpeed ZeRO-3.
No QLoRA, no quantization/dequantization — native precision models only.
...
Usage:
    ./run_accelerate_deepspeed.sh 3 config.json [gpu_ids] modeling_deepspeed_bf16_finetune.py
"""
...
def main():
    ...
    # HfDeepSpeedConfig does NOT resolve "auto" placeholders — only the Trainer's internal
    # HfTrainerDeepSpeedConfig subclass does that later. For non-quantized models,
    # from_pretrained() calls deepspeed.zero.Init() which validates batch sizes before
    # the Trainer exists, so "auto" strings cause a TypeError. Resolve them here.
    dschf = None
    if hasattr(training_args, 'deepspeed') and training_args.deepspeed:
        ...
        if isinstance(training_args.deepspeed, str):
            with open(training_args.deepspeed) as f:
                ds_config_dict = _json.load(f)
        else:
            ds_config_dict = dict(training_args.deepspeed)

        if ds_config_dict.get("train_micro_batch_size_per_gpu") == "auto":
            ds_config_dict["train_micro_batch_size_per_gpu"] = training_args.per_device_train_batch_size
        if ds_config_dict.get("gradient_accumulation_steps") == "auto":
            ds_config_dict["gradient_accumulation_steps"] = training_args.gradient_accumulation_steps
        if ds_config_dict.get("train_batch_size") == "auto":
            ds_config_dict["train_batch_size"] = (
                training_args.per_device_train_batch_size
                * training_args.gradient_accumulation_steps
                * (distributed_state.num_processes if distributed_state is not None else 1)
            )

        dschf = HfDeepSpeedConfig(ds_config_dict)

        # Print comprehensive DeepSpeed config summary
        zero_cfg = ds_config_dict.get("zero_optimization", {})
        offload_opt = zero_cfg.get("offload_optimizer", {})
        offload_param = zero_cfg.get("offload_param", {})
        act_ckpt = ds_config_dict.get("activation_checkpointing", {})
        print("=" * 60)
        print("DeepSpeed Configuration Summary")
        print("=" * 60)
        print(f"  Config source: {training_args.deepspeed}")
        print(f"  --- ZeRO Optimization ---")
        print(f"  ZeRO stage:                  {zero_cfg.get('stage', 'N/A')}")
        print(f"  Offload optimizer:           {offload_opt.get('device', 'none')}"
              f"  (pin_memory={offload_opt.get('pin_memory', 'N/A')})")
        print(f"  Offload params:              {offload_param.get('device', 'none')}"
              f"  (pin_memory={offload_param.get('pin_memory', 'N/A')})")
        print(f"  Overlap comm:                {zero_cfg.get('overlap_comm', 'N/A')}")
        print(f"  Contiguous gradients:        {zero_cfg.get('contiguous_gradients', 'N/A')}")
        print(f"  Reduce bucket size:          {zero_cfg.get('reduce_bucket_size', 'N/A')}")
        print(f"  Prefetch bucket size:        {zero_cfg.get('stage3_prefetch_bucket_size', 'N/A')}")
        print(f"  Param persistence threshold: {zero_cfg.get('stage3_param_persistence_threshold', 'N/A')}")
        print(f"  Gather 16bit on save:        {zero_cfg.get('stage3_gather_16bit_weights_on_model_save', 'N/A')}")
        print(f"  --- Batch Sizes (resolved) ---")
        print(f"  train_micro_batch_size_per_gpu: {ds_config_dict.get('train_micro_batch_size_per_gpu')}")
        print(f"  gradient_accumulation_steps:    {ds_config_dict.get('gradient_accumulation_steps')}")
        print(f"  train_batch_size:               {ds_config_dict.get('train_batch_size')}")
        print(f"  --- Precision ---")
        print(f"  bf16 enabled:                {ds_config_dict.get('bf16', {}).get('enabled', False)}")
        print(f"  fp16 enabled:                {ds_config_dict.get('fp16', {}).get('enabled', False)}")
        print(f"  --- Other ---")
        print(f"  Gradient clipping:           {ds_config_dict.get('gradient_clipping', 'N/A')}")
        print(f"  Activation checkpointing:    {bool(act_ckpt)}")
        if act_ckpt:
            print(f"    partition_activations:      {act_ckpt.get('partition_activations', 'N/A')}")
            print(f"    cpu_checkpointing:          {act_ckpt.get('cpu_checkpointing', 'N/A')}")
            print(f"    contiguous_memory_opt:      {act_ckpt.get('contiguous_memory_optimization', 'N/A')}")
        print("=" * 60)
    ...
    logger.info(f"Loading model: {PRE_TRAINED_MODEL_NAME} with dtype {model_dtype}")
