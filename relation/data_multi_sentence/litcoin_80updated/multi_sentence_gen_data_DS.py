# Generate training json data for multi-sentence format from DS data
import os
import json
import argparse
...
def gen_multi_sentence_data(input_data):
    ...
    Entity information is already in the processed data
    """
    output_data = []
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate actual training data (multi-sentence) for DS format.")
    parser.add_argument("--data_path", action="store", dest="data_path", help="Directory of processed DS data.")
    args = parser.parse_args()

    # Read processed data
    processed_file = os.path.join(args.data_path, "processed", "processed_all.json")
