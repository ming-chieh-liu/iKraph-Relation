...
def convert_new_format_to_old(doc):
    """
    Convert new DS format to old format for processing
    ...
    for rel in doc["Relation_Annotation"]:
        ...
        relation_type = rel["gold_standard"]["relation_type"]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate multi-sentence data for DS format.")
    parser.add_argument("--data_path", action="store", dest="data_path", help="Directory containing DS_with_id folder.")
    args = parser.parse_args()

    # Read all JSON files from DS_with_id directory
    ds_dir = os.path.join(args.data_path, "DS_with_id")
    all_docs = []
